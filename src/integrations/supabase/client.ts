...
const supabaseOptions = {
  ...
  global: {
    ...
    // Fix: Properly type the fetch function return type to Promise<Response>
    fetch: (url: string, options: RequestInit): Promise<Response> => {
      // Check if we're offline before attempting the request
      if (typeof navigator !== 'undefined' && !navigator.onLine) {
        console.log("Device is offline, rejecting request:", url);
        return Promise.reject(new Error("You are currently offline. Please check your internet connection."));
      }

      // Use a timeout to detect potentially slow or blocked connections
      const timeoutPromise = new Promise<Response>((_, reject) => {
        setTimeout(() => {
          reject(new Error("Connection timeout. Unable to reach the server. Please check your internet connection."));
...
const fallbackData = {
  users: [
    { id: 1, email: 'admin@gym.com', name: 'Admin User', role: 'admin' },
    { id: 2, email: 'user@gym.com', name: 'Regular User', role: 'user' },
    { id: 3, email: 'trainer@gym.com', name: 'Trainer User', role: 'trainer' }
  ],
  trainers: [
...
// Check if a user exists in our mock data (for offline login)
export const mockLoginCheck = (email: string, password: string) => {
  // Simple offline login check for demo accounts
  if (email === 'admin@gym.com' && password === 'admin123') {
    return { user: fallbackData.users[0], error: null };
  } 
  if (email === 'user@gym.com' && password === 'user123') {
    return { user: fallbackData.users[1], error: null };
  } 
  if (email === 'trainer@gym.com' && password === 'trainer123') {
    return { user: fallbackData.users[2], error: null };
  }
...
// Utility to check connection to Supabase with a more reliable method
export const checkSupabaseConnection = async () => {
  ...
  try {
    ...
    // Make a lightweight query to check connection with a timeout
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 5000); // 5 second timeout
    
    const response = await fetch(`${SUPABASE_URL}/rest/v1/classes?select=id&limit=1`, {
      method: 'GET',
      headers: {
    ...
    if (error.name === 'AbortError') {
      return {
        connected: false,
        latency: null,
        error: new Error("Connection timeout. The server is taking too long to respond.")
      };
    }
...
// New function to provide a clearer picture of connectivity status
export const getConnectionStatus = async () => {
  // First check basic online status
  const isDeviceOnline = !isOffline();
  ...
  // If device is online, check actual Supabase connectivity
  try {
    const { connected, latency, error } = await checkSupabaseConnection();
    ...
    if (connected) {
      return {
        ...
        message: latency > 1000 
          ? "Connected to server, but connection is slow." 
          : "Connected to server."
      };
    } else {
      return {
        deviceOnline: true,
        supabaseConnected: false,
        message: error?.message || "Unable to connect to the server despite being online. This could be due to firewall restrictions or server issues."
      };
    }
  } catch (e) {
    return {
      deviceOnline: true,
      supabaseConnected: false,
      message: "Error checking connection to server: " + (e.message || "Unknown error")
    };
  }
};
